...
export class UserComponent implements OnInit {
  ...
  async getProfile() {
    ...
    await this.userService.getProfileUser(user).subscribe(async (res: any) => {
  ...
  saveProfile() {
    this.overlayService.open(Constants.OVERLAY_WAIT_SPIN);
    this.userService.updateProfile(this.profile).subscribe((res: any) => {
      if (res.user) {
        this.overlayService.close();
  ...
  savePass() {
    ...
    if (this.currentPass && this.changePass && this.verifyPass && this.changePass === this.verifyPass) {
      ...
      this.overlayService.open(Constants.OVERLAY_WAIT_SPIN);
      this.userService.changePass(object).subscribe((res: any) => {
        if (!res.error) {
          this.overlayService.close();
  ...
  selectCity() {
    ...
    if (this.selectedCity !== null) {
      this.userService.getDistrict(this.selectedCity?.id).map(itemLv1 => {
  ...
  selectDistrict() {
    ...
    if (this.selectedDistrict !== null && this.selectedCity !== null) {
      this.userService.getWard(this.selectedCity?.id, this.selectedDistrict?.id).map(item => {
        item.map(itemWard => {
          const ward = {id: '', name: ''};
  ...
  getListCity() {
    this.listCity = [];
    this.userService.getCity().map(item => {
      const city = {id: '', name: ''};
